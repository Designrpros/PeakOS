...
find ../target -name "peak-native" -type f
BIN_PATH=$(find ../target -name "peak-native" -type f | head -n 1)
...
apk --root /build/rootfs --initdb add --no-cache --allow-untrusted \
...
echo "ttyAMA0::respawn:/sbin/getty -L 0 ttyAMA0 vt100" >> /build/rootfs/etc/inittab
...
menuentry "PeakOS ($ARCH)" {
    linux /boot/vmlinuz root=/dev/ram0 rdinit=/init console=ttyAMA0 console=tty0 earlycon=pl011,mmio32,0x09000000 debug keep_bootcon
    initrd /boot/initrd
}
EOF
...
grub-mkimage \
    -p /boot/grub \
    -o /build/iso/EFI/BOOT/BOOTAA64.EFI \
    -O arm64-efi \
...
mcopy -i /build/iso/efiboot.img /build/iso/EFI/BOOT/BOOTAA64.EFI ::/EFI/BOOT/BOOTAA64.EFI
