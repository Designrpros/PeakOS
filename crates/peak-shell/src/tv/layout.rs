...
pub fn layout<'a, Message>(
    ...
    on_submit: Message,
    map_msg: impl Fn(TvShellMessage) -> Message + 'a + Clone,
) -> Element<'a, Message>
where
    Message: Clone + 'a,
{
    ...
    let map_dock_msg = |msg: crate::dock::DockMessage| match msg {
        crate::dock::DockMessage::Launch(id) => TvShellMessage::Launch(id),
        _ => TvShellMessage::Launch(AppId::Terminal),
    };
    ...
    let home_btn = Element::from(
        ...
        .on_press(TvShellMessage::OpenStart)
        .padding(4)
        .style(move |_, _| iced::widget::button::Style::default()),
    )
    .map(map_msg.clone());

    // TEXT INPUT ("Search TV...")
    let omni_input = text_input("Search TV...", ai_input_value)
        .on_input(on_input)
        .on_submit(on_submit)
        .padding([10, 20])
        .size(14)
